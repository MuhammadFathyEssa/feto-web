...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://feto-agent-production.up.railway.app";
...
export default function ChatPage() {
  ...
  const sendWithFile = useCallback(async (file: File, message: string) => {
    ...
    const res = await fetch(`${API_URL}/api/upload`, { method: "POST", headers: getApiHeaders(), body: formData });
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/transcribe`, { method: "POST", headers: getApiHeaders(), body: formData });
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (file) {
        ...
        data = await sendMessage(USER_ID, text);
      }
  ...
  }, [input, attachedFile, loading, addMessage, sendWithFile]);
    ...
    <div className="flex h-screen bg-[#040d1a] overflow-hidden">
      ...
      <aside className={`fixed md:relative z-30 md:z-auto w-64 h-full flex flex-col bg-[#071428] border-r border-[#0d2144] transition-transform duration-200 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="flex items-center justify-between px-4 py-4 border-b border-[#0d2144]">
          ...
          <button onClick={() => setSidebarOpen(false)} className="md:hidden text-slate-500 hover:text-slate-300"><X size={16} /></button>
        </div>
        <div className="p-3">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center gap-3 px-4 py-3 border-b border-[#0d2144] bg-[#071428]/80 backdrop-blur-sm">
          ...
          <div className="relative">
            <button onClick={() => setAgentDropdown(!agentDropdown)}
          ...
          {activeConv.messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-6 px-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">10 agents · CBE framework · File & voice support</p>
              </div>
        ...
        <div className="px-4 py-3 border-t border-[#0d2144] bg-[#071428]/80 backdrop-blur-sm">
          ...
          <div className="flex items-end gap-2 max-w-4xl mx-auto">
            ...
            {/* Send button */}
            <button onClick={handleSend} disabled={(!input.trim() && !attachedFile) || loading || isRecording}
